...
type Game struct {
	...
	MoveCount    int
	// LiveState includes versioned move history and anonymous seat identities.
	// NULL on legacy rows: those rows must never become new empty games.
	LiveState *string `gorm:"type:jsonb"`
	CreatedAt time.Time
	UpdatedAt time.Time
